public class Coche extends Vehiculo{
    // Consumo por litro y potencia en caballos de fuerza
    private double consumoLitro, potenciaMotor, distancia;

    public Coche(String nombre, double consumoLitro, double potenciaMotor, double distancia){
        super(nombre);
    ...
    public void setConsumoLitro(double consumoLitro){
        if (consumoLitro <= 0){ // Validar que sea mayor a 0
    ...
    public void setPotenciaMotor(double potenciaMotor){
        if (potenciaMotor <= 0){ // Validar que sea mayor a 0
            System.out.println("Error: valor inválido. Debe ser mayor a 0.");
            return;
        }
